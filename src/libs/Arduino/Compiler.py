...
class Compiler:
    ...
    def parseLibs(self, code):
        ...
        if len(initIndexes) >= 1:
            for i in range(len(initIndexes)):
                ...
                lib = lib.replace(' ', '').replace('#include', '').replace('<', '').replace('>', '').replace('"',
                                                                                                             '').replace(
                    '.h', '')
                    ...
                    if len(line) > 0:
                        ...
                        if 'warning:' not in error:
                            ...
                            if errorNum < 0:
                                errorNum += 1
                                errorReport.append({'function': error, 'error': []})
                            errorReport[errorNum]['error'].append(
                                {'line': line, 'error': error, 'bloqsError': bloqsError})
        except:
    ...
    def compileWithMakefile(self, code, board, arduinoDir, sketchbookDir):
        ...
        p = subprocess.Popen('make', shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                             close_fds=(platform.system() != 'Windows'), cwd=self.tmpPath)
    ...
    def compile(self, code, board, arduinoDir, sketchbookDir, target_board_mcu, build_f_cpu):
        ...
        compiler = arduino_compiler.Compiler(self.tmpPath, self.libs, self.core_path, self.ide_path, build_f_cpu,
                                             target_board_mcu)
